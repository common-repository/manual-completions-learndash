...
class manual_completions_learndash {
	...
	function get_enrolled_users() {
		if(!current_user_can("manage_options") || empty($_REQUEST["course_id"]) &&  empty($_REQUEST["group_id"]))
			$this->json_out(array("status" => 0, "message" => __("Invalid Request", 'manual-completions-learndash')));
		...
		$this->json_out(array("status" => 0, "message" => __("Invalid Request", 'manage-completions-learndash')));
	}
